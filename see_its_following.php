...
<div class="row">
  ...
  <?php
    ...
    $sql = "SELECT first_name, username FROM users WHERE id_user = $profile";
    if($query = mysqli_query($conn, $sql)){
        $its_follower = mysqli_fetch_assoc($query);
        if(isset($its_follower["first_name"])){
            $its_fname = $its_follower["first_name"];
            $its_uname = $its_follower["username"];
        }
    }
  ?>
    <h4 class="text-center"><strong><?php
    if(isset($its_fname)){
        echo "Pessoas que <a href='see_profile.php?p=$profile' class='notification-link'><strong>"
        .$its_fname."</strong> (@".$its_uname.")</a> segue";
    }else{
        echo "Seguindo";
    }
     ?></strong></h4><hr>
    <?php
      ...
      $sql = "SELECT * FROM follows
       JOIN users ON follows.following = users.id_user
       WHERE follows.follower = '$profile'
      ...
      if($query = mysqli_query($conn, $sql)){
        ...
        foreach($followers as $follower){
            if(!isset($follower["username"])){
                echo "<h4>Nenhuma pessoa foi encontrada..</h4>";
              }else{
                $id_user2 = $follower["id_user"];
                  echo "<div class='panel panel-default panel-gray'>
                    ...
                    if($id_user != $id_user2){
                        ...
                        $sql = "SELECT * FROM follows WHERE follower = $id_user AND following = $id_user2";
                            if($query2 = mysqli_query($conn, $sql)){
                                ...
                                if(isset($dt["follower"])){
                                    echo "<a title='Deixar de seguir' href='follow.php?u=".$id_user2."&m=unfollow&l=see_its_following.php?p=$profile' 
                                    class='btn btn-warning'>
                                    Deixar de Seguir</a>";
                                }else{
                                    echo "<a title='Seguir' href='follow.php?u=".$id_user2."&m=follow&l=see_its_following.php?p=$profile' 
                        ...
                        echo "<br>@".$follower["username"]."<br><br> ";
                        echo '<a class="btn btn-primary" href="profile.php" role="button">Visitar</a> ';
                    }          
                    echo "</div></div>";
                    ...
                    if($data["id_user"] != $id_user){
                      ...
                      if($id_user != $id_user2){
                          if($query2 = mysqli_query($conn, $sql)){
                              $dt = mysqli_fetch_assoc($query2);
                              if(isset($dt["follower"])){
                                  echo "<a title='Deixar de seguir' href='follow.php?u=".$id_user2."&m=unfits_following.php?p=$profile' 
                                  class='btn btn-warning'>
                                  Deixar de Seguir</a>";
                                }else{
                                    echo "<a title='Seguir' href='follow.php?u=".$id_user2."&m=follow&l=see_its_following.php?p=$profile' 
        ...
        if(count($followers)==0){
                if(isset($its_follower["first_name"])){
                    echo "<h4 class='text-center'><a href='see_profile.php?p=$profile' class='notification-link'><strong>"
                    .$its_follower["first_name"]."</strong> (@".$its_follower["username"].")</a> não está seguindo ninguém...</h4>";
                }else{
                    echo "<h4 class='text-center'>Não foi encontrado nenhuma pessoa...</h4>";
                }
            }
        }
        else{
