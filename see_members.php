...
<div class="row">
  ...
  <?php
    ...
    $sql = "SELECT community_name FROM communities WHERE id_community = $profile";
    if($query = mysqli_query($conn, $sql)){
        $community = mysqli_fetch_assoc($query);
        if(isset($community["community_name"])){
            $community_name = $community["community_name"];
        }
    }
  ?>
    <h4 class="text-center"><strong>Membros <?php
    if(isset($community)){
        echo "de <a href='see_community.php?c=$profile' class='notification-link'><strong>"
        .$community_name."</strong></a>";
    }
     ?></strong></h4><hr>
    <?php

      $sql = "SELECT * FROM is_part_of
       JOIN users ON user = users.id_user
       WHERE community = '$profile'
      ...
      if($query = mysqli_query($conn, $sql)){
                        ...
                        $sql = "SELECT * FROM follows WHERE follower = $id_user AND following = $id_user2";
                            if($query2 = mysqli_query($conn, $sql)){
                                ...
                                if(isset($dt["follower"])){
                                    echo "<a title='Deixar de seguir' href='follow.php?u=".$id_user2."&m=unfollow&l=see_members.php?c=$profile' 
                                    class='btn btn-warning'>
                                    Deixar de Seguir</a>";
                                }else{
                                    echo "<a title='Seguir' href='follow.php?u=".$id_user2."&m=follow&l=see_members.php?c=$profile' 
        ...
        if(count($followers)==0){
                if(isset($community["community_name"])){
                    echo "<h4 class='text-center'><a href='see_community.php?c=$profile' class='notification-link'><strong>"
                    .$community["community_name"]."</strong></a> não possui nenhum membro...</h4>";
                }else{
                    echo "<h4 class='text-center'>Não foi encontrado nenhum membro...</h4>";
                }
            }
        }
        else{
