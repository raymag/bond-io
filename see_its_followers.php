...
<div class="row">
  ...
  ?>
    <h4 class="text-center"><strong>Seguidores <?php
    if(isset($its_fname)){
        echo "de <a href='see_profile.php?p=$profile' class='notification-link'><strong>"
        .$its_fname."</strong> (@".$its_uname.")</a>";
    }
     ?></strong></h4><hr>
    <?php
      ...
      $sql = "SELECT * FROM follows
       ...
       WHERE follows.following = '$profile'
      ...
      if($query = mysqli_query($conn, $sql)){
                        ...
                        $sql = "SELECT * FROM follows WHERE follower = $id_user AND following = $id_user2";
                            if($query2 = mysqli_query($conn, $sql)){
                                ...
                                if(isset($dt["follower"])){
                                    echo "<a title='Deixar de seguir' href='follow.php?u=".$id_user2."&m=unfollow&l=see_its_followers.php' 
                                    class='btn btn-warning'>
                                    Deixar de Seguir</a>";
                                }else{
                                    echo "<a title='Seguir' href='follow.php?u=".$id_user2."&m=follow&l=see_its_followers.php' 
        ...
        if(count($followers)==0){
                if(isset($its_follower["first_name"])){
                    ...
                    .$its_follower["first_name"]."</strong> (@".$its_follower["username"].")</a> não possui nenhum seguidor...</h4>";
                }else{
                    echo "<h4 class='text-center'>Não foi encontrado nenhum seguidor...</h4>";
                }
            }
        }
        else{
